...
from eris.modules.helper_funcs.chat_status import (
    ...
    is_user_in_chat,
    connection_status)
from eris.modules.helper_funcs.extraction import extract_user_and_text
from eris.modules.helper_funcs.string_handling import extract_time
from eris.modules.log_channel import loggable, gloggable
...
def banme(update: Update, context: CallbackContext):
    ...
    try:
        ...
        response_message = "To be honest, we never need you."
    except Exception as e:
        print(e)
        response_message = "Oh no! Something isn't right ! please contact @bloggerminds"
...
def kick(update: Update, context: CallbackContext) -> str:
    ...
    if user_id == context.bot.id:
        message.reply_text("Yeah I'm not gonna do that.")
        return log_message
    ...
    if res:
        context.bot.sendMessage(
            chat.id,
            f"Kicked Out! {mention_html(member.user.id, member.user.first_name)}.",
            parse_mode=ParseMode.HTML)
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"#KICKED\n"
...
def unban(update: Update, context: CallbackContext) -> str:
    ...
    if user_id == context.bot.id:
        message.reply_text("How would I unban myself ?")
        return log_message

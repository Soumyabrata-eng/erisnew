...
import eris.modules.sql.global_bans_sql as sql
from eris.modules.sql.users_sql import get_user_com_chats
from eris import (
    DEV_USERS,
    EVENT_LOGS,
    OWNER_ID,
...
from eris.modules.helper_funcs.extraction import (
    extract_user,
    extract_user_and_text,
)
from eris.modules.helper_funcs.misc import send_to_list
...
def check_and_ban(update, user_id, should_message=True):
    ...
    if user_id in TIGERS or user_id in WOLVES:
        ...
        try:
            spam_watch_ban = spam_watch.get_ban(int(user_id))
        except:
            spam_watch_ban = None
    ...
    if spam_watch_ban:
        ...
        if should_message:
            update.effective_message.reply_text(
                ...
                f"<b>User ID</b>: <code>{spam_watch_ban.id}</code>\n"
                f"<b>Ban Reason</b>: <code>{html.escape(spam_watch_ban.reason)}</code>",
                parse_mode=ParseMode.HTML,
            )
        return

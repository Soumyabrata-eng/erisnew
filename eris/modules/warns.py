...
from eris import dispatcher, BAN_STICKER, WOLVES, DRAGONS
from eris.modules.disable import DisableAbleCommandHandler
from eris.modules.helper_funcs.chat_status import (
    is_user_admin, bot_admin, user_admin_no_reply, user_admin, can_restrict)
from eris.modules.helper_funcs.extraction import extract_text, extract_user_and_text, extract_user
from eris.modules.helper_funcs.filters import CustomFilters
from eris.modules.helper_funcs.misc import split_message
from eris.modules.helper_funcs.string_handling import split_quotes
from eris.modules.log_channel import loggable
from eris.modules.sql import warns_sql as sql
...
def warn(
    ...
    if user.id in DRAGONS:
        if warner:
            message.reply_text("Dragons can't be warned.")
        else:
            message.reply_text("Sorry I can't warn sudo users.")
        return

    if user.id in WOLVES:
        if warner:
            message.reply_text("Wolves are warn immune.")
        else:
            message.reply_text("Sorry I can't warn an whitelisted user.")
        return
    ...
    if num_warns >= limit:
        ...
        message.bot.send_sticker(chat.id, BAN_STICKER)  # eris's sticker
        keyboard = []
        log_reason = (

import json, os, html, datetime, re, sys, traceback, urllib, importlib, platform, time
from psutil import cpu_percent, virtual_memory, disk_usage, boot_time
from platform import python_version
import eris.modules.sql.users_sql as sql
from eris import pbot as app
...
from eris.modules import ALL_MODULES
from eris.modules.helper_funcs.chat_status import is_user_admin
from eris.modules.helper_funcs.misc import paginate_modules
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("eris.modules." + module_name)
...
def test(update: Update, context: CallbackContext):
    update.effective_message.reply_text("Hola tester! I am alive & running perfectly fine.")
...
def start(update: Update, context: CallbackContext):
    ...
    if update.effective_chat.type == "private":
        if len(args) >= 1:
            ...
            sr = update.effective_message.reply_text("Hey how are you sweetie?", parse_mode=ParseMode.MARKDOWN)
            time.sleep(0.5)
            sr.edit_text("⚡")
            time.sleep(0.5)
            sr.edit_text("❤")
            ...
            update.effective_message.reply_photo(
                BOT_IMG,
                caption=PM_START_TEXT.format(escape_markdown(uptime),sql.num_users(),sql.num_chats()),
        ...
        update.effective_message.reply_photo(
            BOT_IMG,
            caption = " I am alive for {}".format(uptime)
       )
...
def settings_button(update: Update, context: CallbackContext):
    ...
    try:
        if mod_match:
            ...
            chat = bot.get_chat(chat_id)
            query.message.reply_text(
                "Hi there! There are quite a few settings for {} - go ahead and pick !"
...
def donate(update: Update, context: CallbackContext):
    ...
    msg.reply_text( 
    "Looks like you're interested in donating me? Please donate my creator instead.",
...
def stats(update, context):
    ...
    update.effective_message.reply_photo(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [                  
                            InlineKeyboardButton(text="Repository",url="https://github.com/Soumyabrata-Eng/ErisNew"),
                            InlineKeyboardButton(text= "Owner", url=f"https://t.me/Subhra_Chatterjee")
                        ],
                    ]
         ),
   )
